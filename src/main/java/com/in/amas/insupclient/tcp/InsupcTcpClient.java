...
public class InsupcTcpClient {
    ...
    // WorkerThreadPool은 순환 참조 방지를 위해 지연 로딩
    private WorkerThreadPool workerThreadPool;
    
    public InsupcTcpClient(InsupcConfig insupcConfig,
                          InsupcProtocolParser insupcProtocolParser,
                          ApplicationContext applicationContext) {
        this.insupcConfig = insupcConfig;
    ...
    @PostConstruct
    public void start() {
        log.info("INSUPC TCP client started");
        ...
        if (clients != null) {
            for (InsupcConfig.InsupcClient clientConfig : clients) {
                ...
                log.info("INSUPC connection pool created - name: {}, host: {}:{}, pool size: {}", 
                        clientConfig.getName(), clientConfig.getHost(), 
                        clientConfig.getPort(), clientConfig.getConnectionPoolSize());
            }
        }
        
        log.info("INSUPC TCP client startup completed - {} connection pools", connectionPools.size());
    }
    ...
    public boolean sendMessage(InsupcMessage message, String requestId) {
        ...
        if (selectedPool == null) {
            log.error("No available INSUPC connection pool - request ID: {}", requestId);
            return false;
        }
    ...
    @PreDestroy
    public void stop() {
        log.info("INSUPC TCP client shutdown started");
        ...
        log.info("INSUPC TCP client shutdown completed");
    }
    
    /**
     * WorkerThreadPool 지연 로딩 (순환 참조 방지)
     */
    private WorkerThreadPool getWorkerThreadPool() {
        if (workerThreadPool == null) {
            workerThreadPool = applicationContext.getBean(WorkerThreadPool.class);
        }
    ...
    private class InsupcConnectionPool {
        ...
        public void initialize() {
            log.info("INSUPC connection pool initialization started - {}", config.getName());
            ...
            initialized = true;
            log.info("INSUPC connection pool initialization completed - {}, connections: {}", 
        ...
        private void createConnection() {
            try {
                ...
                log.debug("INSUPC connection created successfully - {}, channel: {}", config.getName(), channel.id());
                
            } catch (Exception e) {
                log.error("INSUPC connection creation failed - {}, error: {}", config.getName(), e.getMessage(), e);
        ...
        private void sendLogonRequest(Channel channel, InsupcMessage logonRequest) {
            try {
                ...
                log.debug("INSUPC logon request sent - {}, session: {}", 
                        config.getName(), logonRequest.getSessionId());
                
            } catch (Exception e) {
                log.error("INSUPC logon request send failed - {}, error: {}", 
        ...
        public boolean sendMessage(InsupcMessage message, String requestId) {
            ...
            try {
                ...
                if (channel == null || !channel.isActive()) {
                    log.warn("No available INSUPC connection - {}, request ID: {}", 
                ...
                log.info("INSUPC message sent successfully - {}, request ID: {}, code: {}", 
                        config.getName(), requestId, message.getMsgCode());
        ...
        public void shutdown() {
            ...
            log.info("INSUPC connection pool shutdown completed - {}", config.getName());
        }
    }
    ...
    private class InsupcChannelHandler extends ChannelInboundHandlerAdapter {
        ...
        public void channelRead(ChannelHandlerContext ctx, Object msg) throws Exception {
            try {
                ...
                log.debug("INSUPC response received - channel: {}, size: {} bytes", 
                ...
                if (requestId != null) {
                    ...
                    getWorkerThreadPool().submitMessage(workerMessage);
                    
                    // 연결 반환
                    returnConnectionByRequestId(requestId, ctx.channel());
                    
                    log.info("INSUPC response processing completed - request ID: {}, code: {}", 
                            requestId, insupcMessage.getMsgCode());
                } else {
                    log.warn("Cannot find request for INSUPC response - session ID: {}", 
                            insupcMessage.getSessionId());
                }
                
            } catch (Exception e) {
                log.error("Error during INSUPC response processing - channel: {}, error: {}", 
        ...
        public void channelInactive(ChannelHandlerContext ctx) throws Exception {
            log.warn("INSUPC connection disconnected - channel: {}", ctx.channel().id());
            super.channelInactive(ctx);
        }
        
        @Override
        public void exceptionCaught(ChannelHandlerContext ctx, Throwable cause) throws Exception {
            log.error("INSUPC channel exception occurred - channel: {}, error: {}", 
