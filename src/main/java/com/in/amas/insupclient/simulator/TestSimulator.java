...
public class TestSimulator {
    ...
    public CommandLineRunner run() {
        return args -> {
            log.info("=".repeat(60));
            log.info("Incomm-Insup Gateway 테스트 시뮬레이터 시작");
            log.info("=".repeat(60));
    ...
    private void startInsupcServer() {
        CompletableFuture.runAsync(() -> {
            try {
                ...
                log.info("🟢 INSUPC 시뮬레이터 서버 시작 - 포트: {}", INSUPC_PORT);
                
                while (running && !insupcServerSocket.isClosed()) {
                    try {
                        Socket clientSocket = insupcServerSocket.accept();
                        log.info("🔗 Gateway로부터 INSUPC 연결 수락 - {}", clientSocket.getRemoteSocketAddress());
                        ...
                        if (running) {
                            log.error("INSUPC 서버 연결 수락 실패: {}", e.getMessage());
                        }
                    }
                }
                
            } catch (IOException e) {
                log.error("INSUPC 서버 시작 실패: {}", e.getMessage(), e);
            }
        });
    }
    ...
    private void handleInsupcClient(Socket clientSocket) {
        try (DataInputStream dis = new DataInputStream(clientSocket.getInputStream());
            ...
            log.info("📨 INSUPC 클라이언트 처리 시작 - {}", clientSocket.getRemoteSocketAddress());
            ...
            while (running && !clientSocket.isClosed()) {
                try {
                    ...
                    if (messageLength <= 0 || messageLength > 8192) {
                        log.warn("비정상적인 메시지 길이: {}", messageLength);
                        break;
                    }
                    ...
                    log.info("📥 INSUPC 메시지 수신 - 크기: {} bytes", messageLength);
                    
                    // 메시지 파싱 및 응답 처리
                    handleInsupcMessage(messageData, dos);
                    
                } catch (EOFException e) {
                    log.info("클라이언트 연결 종료됨");
                    break;
                } catch (IOException e) {
                    log.error("INSUPC 메시지 처리 중 오류: {}", e.getMessage());
                    break;
                }
            }
            
        } catch (IOException e) {
            log.error("INSUPC 클라이언트 처리 실패: {}", e.getMessage(), e);
        } finally {
            try {
                clientSocket.close();
                log.info("🔌 INSUPC 클라이언트 연결 종료");
            } catch (IOException e) {
                log.warn("소켓 종료 실패: {}", e.getMessage());
            }
        }
    }
    ...
    private void handleInsupcMessage(byte[] messageData, DataOutputStream dos) throws IOException {
        try {
            ...
            log.error("INSUPC 메시지 처리 실패: {}", e.getMessage(), e);
        }
    }
        ...
        CompletableFuture.runAsync(() -> {
            try {
                ...
                try (Socket socket = new Socket(GATEWAY_HOST, GATEWAY_PORT);
                    ...
                    log.info("🔗 Gateway 연결 성공 - {}:{}", GATEWAY_HOST, GATEWAY_PORT);
                    ...
                    log.info("✅ sipsvc 클라이언트 테스트 완료");
                    
                } catch (IOException e) {
                    log.error("sipsvc 클라이언트 테스트 실패: {}", e.getMessage(), e);
                }
                
            } catch (Exception e) {
                log.error("sipsvc 클라이언트 실행 중 오류: {}", e.getMessage(), e);
            }
        });
    }
    ...
    private void sendAuthMessage(DataOutputStream dos) throws IOException {
        ...
        sendJsonMessage(dos, authMessage, "인증");
    }
    ...
    private void sendJsonMessage(DataOutputStream dos, SipsvcMessage message, String messageType) throws IOException {
        try {
            ...
            log.info("📤 {} 메시지 전송 - 크기: {} bytes", messageType, jsonBytes.length);
            log.debug("📤 {} 메시지 내용: {}", messageType, jsonString);
            
        } catch (Exception e) {
            log.error("{} 메시지 전송 실패: {}", messageType, e.getMessage(), e);
    ...
    private void readResponse(DataInputStream dis, String messageType) {
        try {
            ...
            if (responseLength <= 0 || responseLength > 8192) {
                log.warn("{} 응답 길이 비정상: {}", messageType, responseLength);
                return;
            }
            ...
            log.info("📥 {} 응답 수신 - 결과: {}, 메시지: {}", 
                    messageType, response.getResultCode(), response.getResultMessage());
            log.debug("📥 {} 응답 내용: {}", messageType, responseJson);
            
        } catch (Exception e) {
            log.error("{} 응답 읽기 실패: {}", messageType, e.getMessage(), e);
        }
    }
