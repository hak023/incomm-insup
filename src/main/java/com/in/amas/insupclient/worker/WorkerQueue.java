...
public class WorkerQueue {
    ...
    public WorkerQueue(int queueId, int capacity) {
        ...
        log.debug("WorkerQueue {} created successfully - capacity: {}", queueId, capacity);
    }
    ...
    public boolean offer(WorkerMessage message) {
        if (shutdown.get()) {
            log.warn("WorkerQueue {} terminated - message add not allowed: {}", queueId, message.getRequestId());
            return false;
        }
        ...
        if (success) {
            log.debug("Message added to WorkerQueue {} - request ID: {}, current queue size: {}", 
                    queueId, message.getRequestId(), messageQueue.size());
        } else {
            log.warn("WorkerQueue {} saturated - message add failed: {}", queueId, message.getRequestId());
        }
    ...
    public WorkerMessage poll(long timeout) throws InterruptedException {
        ...
        if (message != null) {
            log.debug("Message retrieved from WorkerQueue {} - request ID: {}, remaining queue size: {}", 
                    queueId, message.getRequestId(), messageQueue.size());
        }
    ...
    public void incrementProcessedCount() {
        ...
        if (count % 100 == 0) {
            log.info("WorkerQueue {} processed message count: {}", queueId, count);
        }
    }
    ...
    public void shutdown() {
        shutdown.set(true);
        log.info("WorkerQueue {} terminated - remaining messages: {}, processed messages: {}", 
                queueId, messageQueue.size(), processedCount.get());
        
        // 남은 메시지들을 로그로 출력
        messageQueue.forEach(message -> 
                log.warn("WorkerQueue {} unprocessed message at shutdown - request ID: {}, type: {}", 
