...
public class WorkerThreadPool {
    ...
    public void initialize() {
        log.info("WorkerThreadPool initialization started - thread count: {}, queue capacity: {}", threadPoolSize, queueCapacity);
        ...
        log.info("WorkerThreadPool initialization completed - {} worker threads started", threadPoolSize);
    }
    ...
    public boolean submitMessage(WorkerMessage message) {
        try {
            ...
            if (success) {
                log.debug("Message added to worker queue {} - request ID: {}, type: {}", 
                        queueIndex, message.getRequestId(), message.getMessageType());
            } else {
                log.warn("Worker queue {} saturated - message add failed: {}", queueIndex, message.getRequestId());
                ...
                for (int i = 0; i < threadPoolSize; i++) {
                    ...
                    if (alternativeQueue.offer(message)) {
                        log.info("Message added to alternative worker queue {} - request ID: {}", 
                                alternativeIndex, message.getRequestId());
                        return true;
                    }
                }
                
                log.error("All worker queues saturated - message processing failed: {}", message.getRequestId());
            }
            
            return success;
            
        } catch (Exception e) {
            log.error("Error adding message to worker queue: {}", e.getMessage(), e);
            return false;
        }
    }
    ...
    public void shutdown() {
        log.info("WorkerThreadPool shutdown started");
        ...
        try {
            if (!executorService.awaitTermination(30, TimeUnit.SECONDS)) {
                log.warn("WorkerThreadPool graceful shutdown timeout - force shutdown started");
                executorService.shutdownNow();
                
                if (!executorService.awaitTermination(10, TimeUnit.SECONDS)) {
                    log.error("WorkerThreadPool force shutdown failed");
                }
            }
        } catch (InterruptedException e) {
            log.warn("Interrupt occurred during WorkerThreadPool shutdown");
            executorService.shutdownNow();
            Thread.currentThread().interrupt();
        }
        
        log.info("WorkerThreadPool shutdown completed");
    }
