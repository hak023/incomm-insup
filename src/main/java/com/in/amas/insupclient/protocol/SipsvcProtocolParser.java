...
public class SipsvcProtocolParser {
    ...
    public SipsvcMessage parseMessage(byte[] data) throws Exception {
        try {
            ...
            log.debug(">>> sipsvc message received: {}", jsonString);
            ...
            log.info(">>> sipsvc message parsing completed - type: {}, session: {}, client: {}", 
                    message.getType(), message.getSessionId(), message.getClientIp());
            
            return message;
            
        } catch (Exception e) {
            log.error(">>> sipsvc message parsing failed: {}", e.getMessage(), e);
            throw e;
        }
    }
    ...
    public byte[] serializeMessage(SipsvcMessage message) throws Exception {
        try {
            ...
            log.debug("<<< sipsvc message sent: {}", jsonString);
            
            byte[] data = jsonString.getBytes(StandardCharsets.UTF_8);
            
            log.info("<<< sipsvc message serialization completed - type: {}, session: {}, size: {} bytes", 
                    message.getType(), message.getSessionId(), data.length);
            
            return data;
            
        } catch (Exception e) {
            log.error("<<< sipsvc message serialization failed: {}", e.getMessage(), e);
            throw e;
        }
    }
    ...
    public boolean validateMessage(SipsvcMessage message) {
        ...
        if (SipsvcMessage.Type.EXECUTE.equals(message.getType())) {
            if (message.getRequestId() == null || message.getRequestId().trim().isEmpty()) {
                log.warn("Request ID is missing in execute message.");
                return false;
            }
            if (message.getPhoneNumber() == null || message.getPhoneNumber().trim().isEmpty()) {
                log.warn("Phone number is missing in execute message.");
                return false;
            }
        }
