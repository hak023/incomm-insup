...
#define PLUGIN_NAME "insup2"

static int insup2_on_init(t_plugin_handle * hplug);
static void insup2_on_finish(t_plugin_handle * hplug);
/* Inbound session */
static int insup2_on_iconnect(t_cli_ctx * cli);
static int insup2_on_idisconnect(t_cli_ctx * cli);
static int insup2_on_iidle(t_cli_ctx * cli, apr_time_t elasped_time);
static int insup2_on_irecv(t_cli_ctx * cli, apr_time_t rtime, const char * op, const char * tid, const apr_uint16_t sid, const char * svcid, const char * srcid, Json::Value & root);
/* Outbound session */
static int insup2_on_oconnect(t_cli_ctx * cli, char * conf);
static int insup2_on_odisconnect(t_cli_ctx * cli);
static int insup2_on_oidle(t_cli_ctx * cli, apr_time_t elasped_time);
static int insup2_on_orecv_tokenize(t_cli_ctx * cli, apr_socket_t * csock, char * OUT buf, apr_size_t * IN OUT psize);
static int insup2_on_orecv(t_cli_ctx * cli, apr_time_t rtime, char * IN buf, apr_size_t IN size, int & OUT wantStatistics);

static t_plugin_handle g_plug = {
		PLUGIN_NAME, "",

		insup2_on_iconnect,
		insup2_on_idisconnect,
		insup2_on_iidle,
		insup2_on_irecv,
		NULL /* In */,
		NULL /* In */,
		...
		insup2_on_oconnect,
		insup2_on_odisconnect,
		insup2_on_oidle,
		insup2_on_orecv_tokenize,
		insup2_on_orecv,
		NULL /* In */,
		NULL /* In */,
		NULL /* In */,
 ...
 */
IN_DECLARE(void) parse_insup2_outbound_plugin_info(t_cli_ctx * cli, const char* outbound_plugin_info) {
...
static int insup2_on_oconnect(t_cli_ctx * cli, char * conf) {
    ...
    parse_insup2_outbound_plugin_info(cli, conf);
    ...
    if (result != INSUP_RESULT_SUCCESS) {
        switch(result) {
            case INSUP_RESULT_MODULE_NOT_FOUND:
                LOGE("[%s|%04d] INSUP2 Module not found. Error [%d]\n%s\n", cli->name, cli->sid, size, buf);
                break;
            case INSUP_RESULT_FAIL:
                LOGE("[%s|%04d] INSUP2 General failure. Error [%d]\n%s\n", cli->name, cli->sid, size, buf);
                break;
            case INSUP_RESULT_SENDDATA_FAIL:
                LOGE("[%s|%04d] INSUP2 Send data failure. Error [%d]\n%s\n", cli->name, cli->sid, size, buf);
                break;
            case INSUP_RESULT_INVALID_MESSAGE:
                LOGE("[%s|%04d] INSUP2 Invalid message. Error [%d]\n%s\n", cli->name, cli->sid, size, buf);
                break;
            case INSUP_RESULT_LOGIN_DENIED:
                LOGE("[%s|%04d] INSUP2 Login denied. Error [%d]\n%s\n", cli->name, cli->sid, size, buf);
                break;
            case INSUP_RESULT_OVERLOAD_REJECT:
                LOGE("[%s|%04d] INSUP2 Rejected because overload. Error [%d]\n%s\n", cli->name, cli->sid, size, buf);
                break;
            default:
                LOGE("[%s|%04d] INSUP2. Unknown error(0x%02X) Size: [%d]\n%s\n", cli->name, cli->sid, result, size, buf);
                break;
        }
